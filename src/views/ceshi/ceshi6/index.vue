...
const onBeforeExecCmd = (key: string, command: string, name: string) => {
  console.log('onBeforeExecCmd')
...
const paste = async () => {
  navigator.clipboard.readText().then((data) => {
    TerminalApi.setCommand(
      'my-terminal',
      TerminalApi.getCommand('my-terminal') + data,
    )
  })
}
//字体行间距
...
//监听键盘
const onKeydown = (event: Event, name: string) => {
  console.log('按下的键盘')
  ws.send(event.key)
...
<template>
  <div style="height: 75vh">
    <terminal
      ...
      :enable-fold="setting.logFold"
      :enable-hover-stripe="setting.logHighlight"
      cursor-style="bar"
      :enable-input-tips="setting.inputHint"
      :enable-help-box="setting.shellSampleHint"
      @on-keydown="onKeydown"
      @before-exec-cmd="onBeforeExecCmd"
      log-size-limit="5"
      :line-space="setting.lineSpace"
      :context="currentPwd"
      @exec-cmd="onExecCmd"
      theme="ceshi1"
    >
      ...
      <template #header>
        <div
          ...
          @dblclick.stop="disableDoubleClick()"
        >
          <div
            style="
              display: flex;

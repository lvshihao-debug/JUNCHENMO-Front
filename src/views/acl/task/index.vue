<template>
  <div class="fade">
    <el-card class="searchCard">
      <el-form
        :inline="true"
        :model="sysUserTaskStore.searchForm"
        class="searchForm"
      ...
      >
        <el-row>
          <el-form-item label="任务标题" prop="title">
            <el-input
              v-model="sysUserTaskStore.searchForm.title"
              placeholder="请输入任务标题"
              clearable
              @keyup.enter="refresh"
            />
          ...
          <el-form-item v-show="more" label="任务优先级" prop="priority">
            <el-select
              v-model="sysUserTaskStore.searchForm.priority"
          ...
          <el-form-item v-show="more" label="任务状态" prop="status">
            <el-select
              v-model="sysUserTaskStore.searchForm.status"
              placeholder="请选择任务状态"
              clearable
            >
              <el-option
          ...
          <el-form-item v-show="more" label="任务类型" prop="type">
            <el-select
              v-model="sysUserTaskStore.searchForm.type"
              placeholder="请选择任务类型"
              clearable
            >
              <el-option
          ...
          <el-form-item v-show="more" label="用户名称" prop="userId">
            <el-select
              v-model="sysUserTaskStore.searchForm.userId"
        ...
        <div class="card-header-style">
          <div class="card-header">
            <span class="creative-title">
              <svg-icon name="任务" width="24px" height="24px" class="title-icon" />
              <span class="title-text">用户任务列表</span>
    ...
    <el-card class="card-table-style">
      <el-table
        :data="sysUserTaskStore.dataList.list"
        table-layout="auto"
        ...
        >
          <template #default="scope">
            <template v-if="scope.row.priority === 0">
              <el-tag checked type="info">
                {{ getStrByPriority(scope.row.priority) }}
              </el-tag>
            </template>
            <template v-if="scope.row.priority === 1">
              <el-tag checked type="warning">
                {{ getStrByPriority(scope.row.priority) }}
              </el-tag>
            </template>
            <template v-if="scope.row.priority === 2">
              <el-tag checked type="danger">
                {{ getStrByPriority(scope.row.priority) }}
              </el-tag>
            </template>
          </template>
        ...
        <el-table-column label="状态" align="center" prop="status" width="80">
          <template #default="scope">
            <el-tag checked size="small">
              {{ getStrByStatus(scope.row.status) }}
            </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column align="center" label="操作">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="taskUpdateFromModal?.open(scope.row)"
              text
            >
              编辑
            </el-button>
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="sysUserTaskStore.dataList.total"
...
const deleteItem = (item: any) => {
  sysUserTaskStore.del(item.taskId).then(() => {
    refresh()
  })
}
...
const deleteItems = () => {
  ...
  const taskIds = sysUserTaskStore.multipleSelection
...
const loadingStatus = computed(() => {
  return (
    !sysUserTaskStore.tableLoading || !layoutSettingStore.setting.dataLoading
  )
})
...
.creative-title {
  ...
  .title-text {
    color: transparent;
    background: linear-gradient(120deg, var(--lvshihao-theme-color), #8ec5fc);

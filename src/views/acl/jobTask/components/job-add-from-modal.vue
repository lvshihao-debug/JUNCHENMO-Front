  ...
  <el-dialog v-model="fromOpenStatus" width="500" :show-close="false">
    ...
    >
      <el-form-item label="任务名称" prop="jobName">
        <el-input
          v-model="sysJobStore.commonForm.jobName"
          placeholder="请输入任务名称"
        />
      </el-form-item>
      <el-form-item label="任务分组" prop="jobGroup">
        <el-select
          v-model="sysJobStore.commonForm.jobGroup"
          placeholder="请选择任务组名"
          clearable
        >
          <el-option
      ...
      <el-form-item prop="invokeTarget">
        <template #label>
          ...
          >
            ...
            <el-tooltip effect="dark" placement="top">
              <template #content>
                Bean调用示例：jcmTask.jcmParams('jcm')
                <br />
                Class类调用示例：com.jcm.job.task.JcmTask.jcmParams('jcm')
                <br />
        ...
        <el-input
          v-model="sysJobStore.commonForm.invokeTarget"
      ...
      <el-form-item label="cron表达式" prop="cronExpression">
        ...
        >
          <template #reference>
            <el-input
              v-model="sysJobStore.commonForm.cronExpression"
              placeholder="请输入cron执行表达式"
              readonly
            >
              <template #suffix>
                 <svg-icon name="jobTaskLog" style="cursor: pointer;" />
              </template>
            </el-input>
          </template>
          <JcmCronGenerator v-model="sysJobStore.commonForm.cronExpression" />
        </el-popover>
      </el-form-item>
      <el-form-item label="执行策略" prop="misfirePolicy">
        <el-radio-group
          v-model="sysJobStore.commonForm.misfirePolicy"
          size="small"
        >
          <el-radio-button
            v-for="dict in (instance?.proxy as any).$loadDictDataByName(
              sysJobStore,
              'jobTaskMisfirePolicy',
            )"
            :key="dict.value"
            :label="dict.description"
            :value="dict.value"
      ...
      <el-form-item label="是否并发" prop="concurrent">
        <el-radio-group
          v-model="sysJobStore.commonForm.concurrent"
          size="small"
        >
          <el-radio-button
            v-for="dict in (instance?.proxy as any).$loadDictDataByName(

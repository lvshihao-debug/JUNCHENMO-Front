<template>
  <div>
    <el-card class="searchCard">
      <el-form
        :inline="true"
        :model="dictTypeStore.searchForm"
        class="searchForm"
      ...
      <el-col :span="24">
        <div class="card-header-style">
          ...
          <div class="card-end">
            <JcmButton
              :buttonBgColor="layoutSettingStore.getTheme"
              @click="dictTypeAddFromModal?.open()"
            >
              <template #icon>
                <svg-icon name="加号" />
              </template>
    ...
    <el-card class="card-table-style">
      <el-table
        :data="dictTypeStore.dataList.list"
        table-layout="auto"
        ...
        <el-table-column align="center" label="操作">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="getInfoButtonClick(scope.row)"
              text
            >
              查看
            </el-button>
            <el-button
              size="small"
              type="primary"
              @click="dictTypeUpdateFromModal?.open(scope.row)"
              text
            >
              修改
            </el-button>
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="dictTypeStore.dataList.total"
    ...
    <DictTypeUpdateFromModal
      ref="dictTypeUpdateFromModal"
    ></DictTypeUpdateFromModal>
    <!-- json 查看器模态框 -->
...
const deleteItem = (item: any) => {
  dictTypeStore.deleteDictType(item.dictTypeId).then(() => {
    refresh()
  })
}
...
//删除多个字典类型触发的事件
const deleteItems = () => {
  const dictTypeIds = dictTypeStore.multipleSelection
    .map((item: any) => item.dictTypeId)
    .join(',')
  if (dictTypeStore.multipleSelection.length == 0) {
    ElMessage.warning({ message: '请选择要删除的数据' })
    return
  }
  dictTypeStore.deleteDictType(dictTypeIds).then(() => {
    refresh()
  })
}
...
const getInfoButtonClick = (item: any) => {
  dictDataStore
    ...
    .then((resp) => {
      dictTypeViewFromModal?.value?.open(
        resp.data,

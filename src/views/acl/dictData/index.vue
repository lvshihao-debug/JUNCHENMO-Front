<template>
  <div>
    <el-card class="searchCard">
      <el-form
        :inline="true"
        :model="dictDataStore.searchForm"
        class="searchForm"
      ...
      <el-col :span="24">
        <div class="card-header-style">
          ...
          <div class="card-end">
            <JcmButton
              :buttonBgColor="layoutSettingStore.getTheme"
              @click="dictDataAddFromModal?.open()"
            >
              <template #icon>
                <svg-icon name="加号" />
              </template>
    ...
    <el-card class="card-table-style">
      <el-table
        :data="dictDataStore.dataList.list"
        table-layout="auto"
        ...
        <el-table-column align="center" label="操作" fixed="right">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="dictDataUpdateFromModal?.open(scope.row)"
              text
            >
              修改
            </el-button>
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="dictDataStore.dataList.total"
    ...
    <DictDataUpdateFromModal
      ref="dictDataUpdateFromModal"
    ></DictDataUpdateFromModal>
  </div>
</template>
...
const deleteItem = (item: any) => {
  dictDataStore.deleteDictData(item.dictDataId).then(() => {
    refresh()
  })
}
...
//删除多个字典值触发的事件
const deleteItems = () => {
  const dictDataIds = dictDataStore.multipleSelection
    .map((item: any) => item.dictDataId)
    .join(',')
  if (dictDataStore.multipleSelection.length == 0) {
    ElMessage.warning({ message: '请选择要删除的数据' })
    return
  }
  dictDataStore.deleteDictData(dictDataIds).then(() => {
    refresh()
  })
}

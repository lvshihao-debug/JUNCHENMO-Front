<template>
  ...
  <el-dialog v-model="fromOpenStatus" width="500" :show-close="false">
    ...
    <el-form
      :model="dictDataStore.commonForm"
      label-width="100"
      :rules="formRules"
      ref="formRef"
    >
      <el-form-item label="配置项" prop="name">
        <el-select
          v-model="dictDataStore.commonForm.name"
          filterable
          placeholder="请输入配置项"
        >
          <el-option
            v-for="item in dictDataStore.dictTypeWithExtra"
            :key="item.dictTypeId"
      ...
      <el-form-item label="配置描述" prop="description">
        <el-input
          v-model="dictDataStore.commonForm.description"
          autocomplete="off"
          placeholder="请输入配置描述"
        />
      </el-form-item>
      <el-form-item label="配置值" prop="value">
        <el-input
          v-model="dictDataStore.commonForm.value"
          autocomplete="off"
          placeholder="请输入配置值"
        />
      </el-form-item>
      <el-form-item label="排序" prop="sort">
        <el-input
          v-model="dictDataStore.commonForm.sort"
          autocomplete="off"
          placeholder="请输入排序"
        />
      </el-form-item>
      ...
      <template v-for="item in dictDataStore.selectDictTypeExtra">
        <template v-if="item.type == 0 || item.type == 1 || item.type == 2">
          <el-form-item :label="item.parameter" prop="sort">
            <el-input
              v-model="dictDataStore.extra[item.parameter]"
              autocomplete="off"
...
const open = (item: any) => {
  ...
  const dictTypeObj = dictDataStore.dictTypeWithExtra.find(
    (obj: any) => obj.dictTypeId == item.dictTypeId,
  )
    ...
    if (valid) {
      ...
      for (let key in dictDataStore.extra) {
        if (dictDataStore.extra.hasOwnProperty(key)) {
          ...
          if (
            typeof dictDataStore.extra[key] !== 'boolean' &&
            !dictDataStore.extra[key]
          ) {
      ...
      dictDataStore.upInfoDictData(dictDataStore.commonForm).then(() => {
        const searchQuery = dictDataStore.searchForm
        ;(instance?.proxy as any).$addPage(
          searchQuery,
          dictDataStore.dataList.page,
          dictDataStore.dataList.size,
        )

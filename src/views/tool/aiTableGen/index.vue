    ...
    <el-row>
      <el-col :span="24">
        <div class="form-control">
          <input
            class="table-prompts-input input-alt"
            v-model="prompts"
            placeholder="请输入提示词"
            type="text"
...
const sqlView = () => {
  ...
  aiTableGenStore
    ...
    .then((res) => {
      ...
      sqlEditor?.value?.open(
        format(cleanedString, { language: 'mysql' }),
        '创建表SQL预览',
      )
...
//拖拽触发的事件
const rowDragendEvent: VxeTableEvents.RowDragend = ({
  newRow,
  oldRow,
  dragPos,
}) => {
...
const Gen = (prompts: string) => {
  ...
  aiTableGenStore
    ...
    .then((res) => {
      ...
      // 先去除字符串前后多余的部分（去掉开头的json以及最后的）
      const trimmedString = res.context
        .replace('```json', '')
        .replace('```', '')
      // 再去除可能存在的多余空白字符（比如换行、空格等，使其格式更规范便于解析）
      const cleanedString = trimmedString.trim()

<template>
  <div class="fade">
    ...
    <el-card class="card-table-style">
      <el-table
        :data="genCodeStore.dataList.list"
        table-layout="auto"
        ...
        <el-table-column align="center" label="操作">
          <template #default="scope">
            <el-button
              size="small"
              type="primary"
              @click="previewTableCode(scope.row)"
              text
            >
              预览
            </el-button>
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="genCodeStore.dataList.total"
...
const refresh = async () => {
  ...
  ;(instance?.proxy as any).$addPage(
    searchQuery,
    genCodeStore.dataList.page,
    genCodeStore.dataList.size,
  )
...
const deleteItems = () => {
  ...
  const tableIds = genCodeStore.multipleSelection.map(
    (item: any) => item.tableId,
  )
...
function genTableCode(item: any) {
  ...
  const tbNames =
    (item && item.tableName) ||
    genCodeStore.multipleSelection.map((item: any) => item.tableName).join(',')
  if (tbNames == '') {
  ...
  if (item && item.genType === '1') {
    ...
    ;(instance?.proxy as any).$downloadZip.zip(
      `${API_ENUM.SERVER_MODE_NAME.GEN_CODE}/batchGenCode/?tables=` + tbNames,
      'jcm.zip',
    )
  }
}

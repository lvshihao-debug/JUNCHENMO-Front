<template>
  <div>
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="备注" prop="remark" style="width: 100%;">
              <el-input v-model="genCodeStore.editTableData.table.remark" type="textarea" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
    ...
    <el-card class="card-table-style">
      <vxe-table :edit-config="editConfig" ref="tableRef" :data="genCodeStore.editTableData.columns">
        ...
        <vxe-column field="columnComment" title="字段描述" width="100" show-overflow :edit-render="{}">
          <template #edit="{ row }">
        ...
        <vxe-column field="javaField" title="Java属性" width="105" show-overflow :edit-render="{}">
          <template #edit="{ row }">
        ...
        <vxe-column field="isInsert" title="插入" width="70" :edit-render="{}">
          <template #edit="{ row }">
            <el-checkbox true-label="1" false-label="0" v-model="row.isInsert"></el-checkbox>
          </template>
          <template #default="{ row }">
            <el-checkbox true-label="1" false-label="0" v-model="row.isInsert"></el-checkbox>
          </template>
        ...
        <vxe-column field="isRequired" title="必填" width="70" :edit-render="{}">
          <template #edit="{ row }">
        ...
        <vxe-column field="dictType" title="字典类型" min-width="210" :edit-render="{}">
          <template #edit="{ row }">
            <el-select v-model="row.dictType" style="width: 200px" >
              <el-option label="" value="" />
              <template v-for="item in genCodeStore.dictTypeOptions">
                <el-option :label="(item.name + '-' + item.description)" :value="item.name" />
              </template>
            </el-select>
          </template>
          <template #default="{ row }">
      ...
      <el-form :inline="true" :model="genCodeStore.tableSearchForm" class="searchForm" label-position="right"
        ...
        <el-row>
          ...
          <el-form-item label="生成代码方式" prop="tableName">
            <el-radio-group v-model="genCodeStore.editTableData.table.genType" size="small">
              <el-radio-button label="zip压缩包" value="0" />
              <el-radio-button label="自定义路径" value="1" />
            </el-radio-group>
          </el-form-item>
          <el-form-item label="自定义路径" prop="genPath" v-show="genPathStatus">
    ...
    <el-row style="margin-top: 10px;">
      ...
      <el-col :span="2">
        <JcmButton :buttonBgColor="layoutSettingStore.getTheme" @click="cancelClick()">
          <template #icon>
            <svg-icon name="取消" />
          </template>
        </JcmButton>
      ...
      <el-col :span="2">
        <JcmButton :buttonBgColor="layoutSettingStore.getTheme" @click="saveClick()">
          <template #icon>
            <svg-icon name="数据保存" />
          </template>
        </JcmButton>
...
//保存修改
const saveClick = () => {
  genCodeStore.updateTableColumns().then(()=>{

<template>
  <div>
    <el-card class="searchCard">
      <el-form
        :inline="true"
        :model="operationLogStore.searchForm"
        class="searchForm"
      ...
      >
        <el-row style="display: flex">
          ...
          <el-form-item label="操作模块" prop="title">
            <el-select
              v-model="operationLogStore.searchForm.title"
              @change="changeTitle"
            >
          ...
          <el-form-item label="业务名称" prop="businessName">
            <el-select v-model="operationLogStore.searchForm.businessName">
              <template
                v-for="businessName in operationLogStore.businessNameOptionSelect"
              >
    ...
    <el-card class="card-table-style">
      <el-table
        :data="operationLogStore.dataList.list"
        table-layout="auto"
      ...
      >
        ...
        <el-table-column label="模块/业务" align="center">
          <template #default="scope">
            <span>{{ scope.row.title }} / {{ scope.row.businessName }}</span>
            <br />
          </template>
        </el-table-column>
        <el-table-column prop="status" label="执行结果" align="center">
          <template #default="scope">
            <template v-if="scope.row.status === 0">
              <el-tag checked size="small">成功</el-tag>
            </template>
            <template v-if="scope.row.status === 1">
              <el-tag checked size="small" color="#393e46">失败</el-tag>
            </template>
          </template>
        </el-table-column>
        <el-table-column prop="description" label="操作日志" align="center">
          <template #default="scope">
            ...
            >
              <div>
                <span
                  v-html="scope.row.descriptionHtml"
                  @click="
                    (instance?.proxy as any).$copyText(scope.row.description)
                  "
                  class="copy-span"
                ></span>
              </div>
              <template v-if="scope.row.errorMsgHtml != null">
                <div class="ellipsis-text">
                  <span
                    v-html="scope.row.errorMsgHtml"
                    @click="
                      (instance?.proxy as any).$copyText(scope.row.errorMsg)
                    "
                    class="copy-span"
                  ></span>
                </div>
        ...
        <el-table-column label="留痕地址" align="center">
          <template #default="scope">
            <span style="font-size: 13px">{{ scope.row.operLocation }}</span>
            <br />
            <span
              @click="(instance?.proxy as any).$copyText(scope.row.operIp)"
              class="copy-span"
              style="font-size: 12px; color: rgb(134 144 156)"
            >
              {{ scope.row.operIp }}
            </span>
          </template>
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="operationLogStore.dataList.total"
...
const refresh = async () => {
  ...
  ;(instance?.proxy as any).$addPage(
    searchQuery,
    operationLogStore.dataList.page,
    operationLogStore.dataList.size,
  )
  ;(instance?.proxy as any).$addDateRange(
    searchQuery,
...
const loadingStatus = computed(() => {
  return (
    !operationLogStore.tableLoading || !layoutSettingStore.setting.dataLoading
  )
})
...
const deleteItem = (item: any) => {
  operationLogStore.deleteOperationLog(item.operId).then(() => {
    refresh()
  })
}
...
//删除多个操作日志触发的事件
const deleteItems = () => {
  const operIds = operationLogStore.multipleSelection
    .map((item: any) => item.operId)
    .join(',')
...
const exportData = () => {
  ...
  ;(instance?.proxy as any).$download(
    `${API_ENUM.SERVER_MODE_NAME.OPERACTION_LOG_DATA}/export`,
    {
      ...searchQuery,
    },

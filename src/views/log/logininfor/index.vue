<template>
  <div class="fade">
    <el-card class="searchCard">
      <el-form
        :inline="true"
        :model="sysLogininforStore.searchForm"
        class="searchForm"
      ...
      >
        <el-row>
          <el-form-item label="用户账号" prop="userName">
            <el-input
              v-model="sysLogininforStore.searchForm.userName"
              placeholder="请输入用户账号"
              clearable
              @keyup.enter="refresh"
            />
          ...
          <el-form-item label="登录IP地址" prop="ipaddr">
            <el-input
              v-model="sysLogininforStore.searchForm.ipaddr"
          ...
          <el-form-item label="登录状态" prop="status">
            <el-select
              v-model="sysLogininforStore.searchForm.status"
              placeholder="请选择登录状态"
              clearable
            >
              <el-option
          ...
          <el-form-item v-show="more" label="操作系统" prop="os">
            <el-input
              v-model="sysLogininforStore.searchForm.os"
              placeholder="请输入操作系统"
              clearable
              @keyup.enter="refresh"
            />
    ...
    <el-card class="card-table-style">
      <el-table
        :data="sysLogininforStore.dataList.list"
        table-layout="auto"
      ...
      >
        ...
        <el-table-column label="留痕地址" align="center" prop="ipaddr">
          <template #default="scope">
            <span style="font-size: 13px">{{ scope.row.loginLocation }}</span>
            <br />
            <span
              @click="(instance?.proxy as any).$copyText(scope.row.ipaddr)"
              class="copy-span"
      ...
      <template #footer>
        <div class="pagination-style">
          ...
          <el-pagination
            ...
            :total="sysLogininforStore.dataList.total"
...
const deleteItems = () => {
  ...
  const infoIds = sysLogininforStore.multipleSelection
    .map((item: any) => item.infoId)
    .join(',')
...
const loadingStatus = computed(() => {
  return (
    !sysLogininforStore.tableLoading || !layoutSettingStore.setting.dataLoading
  )
})
</script>
<script lang="ts">
